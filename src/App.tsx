...
export default function App() {
  ...
  const handleCalculateRoute = async (
    ...
    opts?: { start?: GeoPoint | null; end?: GeoPoint | null; simulate?: boolean },
  ) => {
    // 最新mainの points 指定（直接入力）と事故シミュレーション override を両立：
    // opts.start/end があればそれを優先、なければ state の routeStart/routeEnd を使う
    const start = opts?.start !== undefined ? opts.start : routeStart;
    const end = opts?.end !== undefined ? opts.end : routeEnd;
    if (!start || !end) return;
    setIsRoutingLoading(true);
    const profile = profileOverride ?? driverProfile;
    const vehicle = vehicleOverride ?? vehicleType;
    const simulate = opts?.simulate ?? simulateAccident;
